...
type OrderGoods struct {//订单商品表
	...
	Count 		int		`orm:"default(1)"`	//单个商品数量
	Price 		int							//商品价格小计
...
func init() {
	...
	// register model
	orm.RegisterModel(new(User),new(Address),new(Goods),new(GoodsType),
		new(GoodsImage),new(GoodsSKU),new(IndexGoodsBanner),
		new(IndexTypeGoodsBanner),new(IndexPromotionBanner),
